...
### GGMAP
if (is.null(myMap2 )) { 
    myMap2 <- get_stamenmap(bbox=sp::bbox(as_Spatial(inat_sf)), zoom = 9, maptype="toner-lines", crop=TRUE)
    ggmap(myMap2)
}
...
int_df =  cbind(MCSTTotal = mcst_total_dt,MCSTLocal = mcst_local_dt,  MCSTforeign = mcst_foreign_dt, iNatTotal = inat_total_v, inat_df)
